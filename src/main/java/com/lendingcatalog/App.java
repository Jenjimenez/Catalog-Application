...
public class App {
    ...
    private void initialize() {
        ...
        String fileName = FILE_BASE_PATH + "catalog.txt";
       try (BufferedReader br = new BufferedReader(new FileReader(fileName))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                List<CatalogItem> ownerItems = catalog.getOrDefault(owner, new ArrayList<>());
                CatalogItem item = new CatalogItem(name, creator, year, owner) {
                    @Override
       ...
       try {
            List<String> newList = FileStorageService.readContentsOfFile("src/main/resources/members.dat");
            for (String member : newList) {
               String[] memberInfo = member.split(FIELD_DELIMITER);
                Member newMember = new Member(memberInfo[0], memberInfo[1]);
                List<String> memberItems = FileStorageService.readContentsOfFile(FILE_BASE_PATH + memberInfo[2]);
                List<CatalogItem> memberItemList = new ArrayList<>();
                for (String item : memberInfo) {
                    String[] itemInfo = item.split(FIELD_DELIMITER);
                    if (itemInfo.length != 4) {
                        throw new FileStorageException("Item has too many or too few fields");
                    } else if (itemInfo[0].equals("movie")) {
                        createMovie(newMember);
    ...
    public void createMovie(Member member) throws FileStorageException {
        LocalDate localDate = LocalDate.parse(item[3]);
        Movie newMovie = new Movie(item[1], item[2], localDate);
        memberItems.add(newMovie);
